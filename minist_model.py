...
class ResNetCifar10(nn.Module):
    ...
    def __init__(self, n_block=3):
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv1', nn.Conv2d(1, ch[0], 3, 1, 1, bias=False)), # 1x28x28->4x28x28
            ('norm1', nn.BatchNorm2d(ch[0])),
            ('relu1', nn.ReLU(inplace=True)),
            ('resb1', ResidualBlock(ch[0], ch[0], n_block)), #4x28x28->4x28x28
            ('resb2', ResidualBlock(ch[0], ch[1], n_block, 2)), # 4x28x28->8x14x14
            ('resb3', ResidualBlock(ch[1], ch[2], n_block, 2)), # 8x14x14->16x7x7
            ('avgpl', nn.AvgPool2d(7)),
        ]))
